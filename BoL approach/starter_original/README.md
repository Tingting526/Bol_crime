...
## Child/Young Adult crime sample from this project

For the current seminar crime project, `create_child_bol_sample.py`
creates a small API-free Book-of-Life sample directly from the cleaned
NLSY79 Child/Young Adult feature subset already produced in the parent
folder:

```bash
python create_child_bol_sample.py
```

It writes `data/processed_child_crime/books_child_crime_sample.json`
plus matching target labels and a feature index. Crime outcomes are kept
out of the rendered book text and saved separately in
`child_crime_targets_sample.csv`.
